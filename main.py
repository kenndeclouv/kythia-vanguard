#!/usr/bin/env python3
"""
🍪 KENN-RECON Pro v1.0.0-rc.1 — Advanced Recon & Security Auditor
...
# ── src imports (Hanya Import Core, Module di-load dinamis!)
from src.config import console, C, rate_limiter, SCAN_MODULES, SHODAN_API_KEY, HIBP_API_KEY
from src.models import ScanResult
from src.ui.banner import show_banner
from src.ui.menu import interactive_menu
import src.ui.display as display_ui  # Import seluruh isi file display
...
# ─────────────────────────────────────────────────────────────────
# Scan orchestrator (The Autoload Magic 🪄)
# ─────────────────────────────────────────────────────────────────
...
def run_scan(target_raw: str, modules_selected: list[str]) -> None:
    ...
    # 1. 🪄 AUTOLOAD RUNNERS: Bangun MODULE_MAP secara otomatis dari config.py
    MODULE_MAP = {}
    for mod_id, mod_desc in SCAN_MODULES:
        if mod_id not in modules_selected: continue
        
        short_name = mod_desc.split("—")[0].strip()
        
        try:
            # Import file dari src/modules/ sesuai ID di config
            mod_obj = importlib.import_module(f"src.modules.{mod_id}")
            
            # Cari fungsi yang namanya berawalan 'run_' di file tersebut
            runner_func = None
            ...
            if runner_func:
                # Karena tiap run_xxx parameternya beda (ada yg butuh hostname, ada yg nggak)
                # Kita inspect aja parameternya, lalu injeksi data yang sesuai!
                sig = inspect.signature(runner_func)
                
                def make_runner(run_f, signature):
                    def wrapper(p, t):
                        kwargs = {}
                        if 'target_url' in signature.parameters: kwargs['target_url'] = target_url
                        if 'hostname'   in signature.parameters: kwargs['hostname'] = hostname
                        if 'result'     in signature.parameters: kwargs['result'] = result
                        if 'progress'   in signature.parameters: kwargs['progress'] = p
                        if 'task'       in signature.parameters: kwargs['task'] = t
                        return run_f(**kwargs)
                    return wrapper

                MODULE_MAP[mod_id] = (short_name, 50, make_runner(runner_func, sig))
            else:
                console.print(f"[{C['warn']}]⚠ Fungsi run_ tidak ditemukan di module {mod_id}[/{C['warn']}]")
                
        except Exception as e:
            console.print(f"[{C['bad']}]✗ Error memuat module {mod_id}: {e}[/{C['bad']}]")


    # 2. 🚀 JALANKAN MODULE (Yang aktif aja)
    with Progress(
        ...
        for mod_id in modules_selected:
            if mod_id not in MODULE_MAP: continue
            
            name, total, runner = MODULE_MAP[mod_id]
            task = progress.add_task(f"[cyan]{name}[/cyan]", total=total)
            try:
                runner(progress, task)
                progress.update(task, completed=total, description=f"[green]✓ {name} complete[/green]")
    ...
    # 3. 🪄 AUTOLOAD DISPLAY: Cari fungsi UI secara otomatis
    for mod_id in modules_selected:
        if mod_id not in MODULE_MAP: continue
        
        # Cari fungsi di src.ui.display yang berawalan "display_" 
        # dan mengandung ID module (ex: mod_id "fuzz" bakal nemu "display_fuzzing")
        disp_func = None
        for name, obj in inspect.getmembers(display_ui, inspect.isfunction):
            if name.startswith("display_") and (mod_id in name or name.replace("display_", "") in mod_id):
                disp_func = obj
                break
    ...
    # Tetap jalankan display_score sebagai penutup
    ...
    console.print()
    console.print(Rule("[dim]KENN-RECON Pro v1.0.0-rc.1 — Scan Complete[/dim]", style="dim"))
    console.print()


# ─────────────────────────────────────────────────────────────────
# Interactive entry point & CLI (Tidak Berubah)
# ─────────────────────────────────────────────────────────────────
...
def _build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="KENN-RECON Pro v1.0.0-rc.1 — Advanced Recon & Security Auditor")
    ...
    quick.add_argument("--modules", "-m", default="recon,waf,headers,ports,osint,cve,spider,nuclei", help="Comma-separated module list (default: all)")
