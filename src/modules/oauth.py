...
def run_oauth(
    ...
    progress.update(
        task,
        description=f"[cyan]OAuth Hunter:[/cyan] Testing {len(oauth_urls)} OAuth flows…",
    )
    ...
    progress.update(task, completed=100)
    score_and_report(result, "oauth")


def score_oauth(result):
    if not result.oauth_findings:
        return 100
    return max(0, 100 - min(len(result.oauth_findings) * 20, 80))
...
def export_oauth(result: ScanResult, W: callable) -> None:
    if result.oauth_findings:
        W("## 🔐 OAuth Findings\n\n")
        for f in result.oauth_findings:
            W(f"- **Issue**: `{f.get('url', '?')}`\n")
        W("\n")
