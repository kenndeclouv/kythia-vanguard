...
def run_apisec(target_url: str, hostname: str, result: ScanResult, progress, task) -> None:
...
def display_apisec(result: ScanResult) -> None:
    ...
    for f in findings:
        ...
        for ep in f.get("endpoints", []):
            color = "green" if ep['method'] == "GET" else "red" if ep['method'] in ["POST", "DELETE"] else "yellow"
            tbl.add_row(
        ...
        if f['endpoints_count'] > len(f.get("endpoints", [])):
             console.print(f"  [dim]...and {f['endpoints_count'] - len(f['endpoints'])} more endpoints truncated.[/dim]")
